...
def mailQuery(query):
    ...
    while not recipient_found:
        ...
        if any(word in ["no", "cancel", "not", "don't",'doesn\'t'] for word in recipient.split()):
            ...
            recipient_email = ""
            with open(r'C:\DSA\Shadow-Voice_Assistant\Shadow-Voice_Assistant\ShadowGui\features\secure\recipients.csv', mode='r') as csv_file:
...
def sendEmail(recipient_email, message): 
    try:
        with open(r'C:\DSA\Shadow-Voice_Assistant\Shadow-Voice_Assistant\ShadowGui\features\secure\email.txt') as f :
